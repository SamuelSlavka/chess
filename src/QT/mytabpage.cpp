...
MyTabPage::MyTabPage(QWidget *parent) : QWidget(parent),
                                        ui(new Ui::MyTabPage)
{
    CurrentRow = -1;
    CurrentMove = 0;
    ...
    for (int x = 0; x < 8; x++)
    {
        for (int y = 0; y < 8; y++)
        {
            ...
            if ((x + y) % 2)
                m_myWidget->setStyleSheet(" background-color: rgb(204, 102, 0);");
            else
                m_myWidget->setStyleSheet(" background-color: rgb(255, 204, 102);");
