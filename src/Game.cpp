...
void Game::MakeMove(int x, int y)
{
    ...
    if (check != 'n' ) {
        if ( check == 'M' ) MoveList.back().Mat = it->Mat = true;
        else MoveList.back().Chess = it->Chess = true;
    }
...
void Game::GetMoves(int x, int y)
{
    ...
    else if ( !PossibleMoves.empty() ) PossibleMoves.clear();
...
void Game::RemoveNonCheckMoves()
{
    for( auto it = PossibleMoves.begin(); it != PossibleMoves.end();)
    {
        ...
        UnMakeTestMove(*it);
        if(flag != 'n') it = PossibleMoves.erase(it);
        else it++;
    }
}
