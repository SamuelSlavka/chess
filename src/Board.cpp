...
void Board::SetKing( PieceColor turn , int x, int y )
{
    ...
    else BlackKing = &(board.at( static_cast<size_t>(x+y*8) ));
}
...
void Board::UpdateKing(bool un, Move& move )
{
    ...
    else BlackKing = &board.at( static_cast<size_t>( x + y ) );

}

void Board::UnMovePiece(Move& move)
{
    if ( move.Moved->GetPieceType() == 'K' ) UpdateKing(false, move );
    ...
    else board.at( static_cast<size_t>( move.FromX + move.FromY*8 )).piece = move.Moved;
    board.at( static_cast<size_t>( move.FromX + move.FromY*8 )).Used = true;

    if ( move.Removes ) { board.at( static_cast<size_t>( move.ToX + move.ToY*8 )).piece = move.Removed;
        board.at( static_cast<size_t>( move.ToX + move.ToY*8 )).Used = true;
    }
    else{  board.at( static_cast<size_t>( move.ToX + move.ToY*8 )).piece = nullptr;
...
void Board::MovePiece(Move& move)
{
    if ( move.Castle ){
        ...
        if ( move.Moved->GetPieceType() == 'K' ) UpdateKing(true, move );
